...
public class PersonDAO implements BaseDAO<Person> {
		...
		try (Connection connection = ConnectionManager.getConnection()) {
			...
			while (rs.next()) {
				Person p = new Person(rs.getInt("id"), rs.getString("name"), rs.getString("lastname"),
						rs.getString("workphone"), rs.getString("address"), rs.getString("homephone"),
						rs.getString("email"), rs.getString("city"), rs.getString("birthdate"),
	...
	public int update(Person item) {
		...
		try (Connection conn = ConnectionManager.getConnection()) {
			...
			String query = "update person set name=?, lastname=?, workphone=?, address=?, homephone=?, email=?, city=?, birthdate=?, category_id=? where id=?";
			...
			LocalDate brth = LocalDate.of(1980, 01, 01);
			psmt.setDate(8, Date.valueOf(brth));
